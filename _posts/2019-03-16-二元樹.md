...
```
先序遍歷順序：根節點-->左子樹-->右子樹。遍歷過程如下圖所示,圖出自"Data Structures and Algorithms:
...
```
中序遍歷順序:左子樹-->根節點-->右子樹，如下圖所示,圖出自"Data Structures and Algorithms:
...
```
後序遍歷，順序：左子樹-->右子樹-->根節點，如下圖所示,圖出自"Data Structures and Algorithms:
...
**4.4按樹的深度遍歷**
即每次遍歷完成一層的深度後再往下遍歷之。需要std::queue輔助保存當前遍歷節點之位置;std::vector保存所遍歷的每一個節點的值，這樣vector裏的數據就會按由小到大的順序排列。
```c++
