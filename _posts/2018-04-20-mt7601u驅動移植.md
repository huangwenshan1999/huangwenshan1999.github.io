...
然後編譯，成功後驅動在mt7601u-ap/os/linux/mt7601Uap.ko,再在/etc下建立wifi/RT2870AP文件夾，把RT2870AP.txt文件複製到此文件夾內。
```
...
```
則需要修改linux-xlnx根目錄下的Makefile文件，在817行處修改
```
把
...
```
但 是在我這裏，成功編譯驅動並且加載了，但是後面啓動無線適配器時卻工作不正常，即開啓AP模式後，使用iwconfig查看時，生成了很多個interface,從ra0 - ra15，essid是HT_AP0,HT_AP1.....然而在手機wifi連接界面上只看到了HT_AP0,連接會顯示失敗。
...
```
修改保存，重新編譯。在啓動開發板之前，需要在uboot界面修改bootargs參數
```
setenv bootargs 'root=/dev/raw rw cma=25M coherent_pool=2M  init=/linuxrc'
```
這裏的啓動方式是q-spi啓動，如果是其他啓動方式則要進行相應修改;但是coherent_pool=2M一定要有，否則驅動加載會出錯。cma=25M也添加進去吧，我懶得驗證了。沒有正確設置bootargs參數之前，加載驅動時會出現如下錯誤：
```
